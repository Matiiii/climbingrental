...
public class ProductServiceImpl implements ProductService {
    ...
    @Autowired
    public ProductServiceImpl(ProductRepository productRepository, ProductOrderService productOrderService, SessionService sessionService) {
    ...
    public List<CountProducts> createListOfCountProduct(List<ProductEntity> products) {
        if (sessionService.getCurrentUser() != null) {
            if (sessionService.getCurrentUser().getRole().getRole().equals("ROLE_ADMIN") || sessionService.getCurrentUser().getRole().equals("ROLE_STAFF")) {
                ...
                return products.stream().filter(productEntity -> productEntity.getMember() && productEntity.getAvailable()).map(productEntity -> new CountProducts(productEntity, productEntity.getQuantity())).collect(Collectors.toList());
            }
        }
